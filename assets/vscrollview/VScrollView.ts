...
// 子项出现上下文
export interface ItemAppearContext {
  /** 是否属于列表当前的初始出现批次 */
...
// 子项显示前初始化回调
export type OnItemInitFn = (node: Node, index: number, context?: ItemAppearContext) => void;
// 新增项首次布局时的入场动画回调（已废弃，请使用 onItemEdgeEnterFn）
export type PlayItemAppearAnimationFn = (node: Node, index: number, context?: ItemAppearContext) => void;
// 子项边缘进入可视区时回调
export type OnItemEdgeEnterFn = (node: Node, index: number, context?: ItemAppearContext) => void;
// 子项完全进入可视区时回调
export type OnItemFullEnterFn = (node: Node, index: number, context?: ItemAppearContext) => void;
// 返回指定索引子项的主轴尺寸
...
export class VirtualScrollView extends Component {
  ...
  @property({
    displayName: '可视区回调仅触发一次',
    tooltip: '开启后，每个 item 的边缘进入/完全进入可视区回调各自只会触发一次，离开后再次进入也不会重复触发',
    visible(this: VirtualScrollView) {
      return this.useVirtualList;
    },
  })
  public triggerItemEnterCallbackOnce: boolean = false;
  ...
  @property({
    ...
    range: [0, 10, 0.5],
    visible(this: VirtualScrollView) {
      return this.scrollStyle === ScrollStyle.CUSTOM;
    },
  })
  ...
  public enableMouseWheel: boolean = false;
  ...
  // 子项显示前初始化事件
  public onItemInitFn: OnItemInitFn | null = null;
  // 新增项首次布局动画
  ...
  private _initTriggeredIndices: Set<number> = new Set();
  private _edgeTriggeredIndices: Set<number> = new Set();
  private _fullyTriggeredIndices: Set<number> = new Set();
  private _skipSnapToEndOnce: boolean = false;
  private _isInitialAppearBatch: boolean = false;
  private _initialAppearSequence: number = 0;
  private _initialAppearOrders: Map<number, number> = new Map();
  private _endInitialAppearBatch = () => {
    this._isInitialAppearBatch = false;
  ...
  private _hasScrollableRange(epsilon: number = 2): boolean {
    return Math.abs(this._boundsMax - this._boundsMin) > epsilon;
  }
  ...
  async start() {
    ...
    if (!this.onItemEdgeEnterFn && this.playItemAppearAnimationFn) {
      this.onItemEdgeEnterFn = this.playItemAppearAnimationFn;
      this.playItemAppearAnimationFn = null;
      console.warn('[VirtualScrollView] playItemAppearAnimationFn 已废弃，请直接使用 onItemEdgeEnterFn 来实现子项入场动画');
    }
  ...
  onDestroy() {
    ...
    this._resetItemEnterCallbackState();
    this._releaseNestedTouchOwner();
  ...
  private _beginInitialAppearBatch() {
    this._isInitialAppearBatch = true;
    this._initialAppearSequence = 0;
    this._initialAppearOrders.clear();
    this.unschedule(this._endInitialAppearBatch);
    this.scheduleOnce(this._endInitialAppearBatch, 0);
  }

  public skipSnapToEndOnce() {
    ...
    for (let slot = 0; slot < this._slots; slot++) {
      ...
      if (!this._edgeVisibleIndices.has(index) && !hasTriggeredEdge && this.onItemEdgeEnterFn) {
        ...
        if (this.triggerItemEnterCallbackOnce) {
          this._edgeTriggeredIndices.add(index);
        }
      }
      ...
      if (!this._fullyVisibleIndices.has(index) && !hasTriggeredFull && this.onItemFullEnterFn) {
        ...
        if (this.triggerItemEnterCallbackOnce) {
          this._fullyTriggeredIndices.add(index);
        }
      }
    }
  ...
  private _scrollToPosition(targetPos: number, animate = false, duration?: number, onComplete?: () => void) {
  ...
  public scrollToTop(animate = false, duration?: number, onComplete?: () => void) {
    if (this._runOrQueueAfterStart(() => this.scrollToTop(animate, duration, onComplete))) return;
    const target = this._isVertical() ? this._boundsMin : this._boundsMax;
  ...
  public scrollToIndex(index: number, animate = false, duration?: number, onComplete?: () => void, center = false) {
    if (this._runOrQueueAfterStart(() => this.scrollToIndex(index, animate, duration, onComplete, center))) return;
    ...
    if (this.useDynamicSize) {
      ...
      targetPos = center ? itemStart - this._viewportSize / 2 + itemSize / 2 : itemStart;
    } else {
      // 等大小模式：需要手动加上 headerSpacing
      const line = Math.floor(index / this.gridCount);
      const itemStart = this.headerSpacing + line * (this.itemMainSize + this.spacing);
      targetPos = center ? itemStart - this._viewportSize / 2 + this.itemMainSize / 2 : itemStart;
    }
  ...
  private async _layoutSingleSlot(node: Node | null, idx: number, slot: number) {
    ...
    if (this.useDynamicSize) {
      ...
      if (this._needAnimateIndices.has(idx)) {
        if (this.playItemAppearAnimationFn) this.playItemAppearAnimationFn(newNode, idx, appearContext);
      ...
      if (shouldInit) this._prepareItemBeforeShow(node, idx, appearContext);
      else node.active = true;
      if (this._needAnimateIndices.has(idx)) {
        if (this.playItemAppearAnimationFn) this.playItemAppearAnimationFn(node, idx, appearContext);
  ...
  private _createItemAppearContext(index: number, isReused: boolean): ItemAppearContext {
    let appearOrder = 0;
    if (this._isInitialAppearBatch) {
      if (!this._initialAppearOrders.has(index)) {
        this._initialAppearOrders.set(index, this._initialAppearSequence++);
      }
      appearOrder = this._initialAppearOrders.get(index) ?? 0;
    }

    return {
      isInitialBatch: this._isInitialAppearBatch,
      appearOrder,
      isReused,
    };
  }

  private _prepareItemBeforeShow(node: Node, index: number, context: ItemAppearContext) {
    node.active = false;
    const hasTriggeredInit = this.triggerItemEnterCallbackOnce && this._initTriggeredIndices.has(index);
    if (this.onItemInitFn && !hasTriggeredInit) {
      this.onItemInitFn(node, index, context);
      if (this.triggerItemEnterCallbackOnce) {
        this._initTriggeredIndices.add(index);
      }
    }
    node.active = true;
  }
  ...
  /**
   * 根据数据索引获取当前已渲染的节点
   * 如果该索引不在可视区范围内则返回 null
   */
  public getItemByIndex(index: number): Node | null {
    if (!this.useVirtualList) return null;
    if (index < 0 || index >= this.totalCount) return null;
    const slot = index - this._slotFirstIndex;
    if (slot < 0 || slot >= this._slots) return null;
    const node = this._slotNodes[slot];
   ...
   * 滚动到指定页
   */
  public scrollToPage(pageIndex: number, animate: boolean = true, onComplete?: () => void) {
    if (!this.enablePageSnap) {
